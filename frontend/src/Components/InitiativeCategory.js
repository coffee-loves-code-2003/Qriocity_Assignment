...
import {getinitiative} from '../actions/initiativeActions'
function InitiativeCategory() {
  const {category}=useParams();
  console.log(category);
  const dispatch=useDispatch();
  const {initiative,loading}=useSelector(state=>state.initiativeState);
    useEffect(()=>
    {
        dispatch(getinitiative(category))
    },[dispatch,category])
  return (
    <div>
          <div>
      ...
      {loading?(<Loader/>):(
        ...
        {initiative && initiative.map(algo=>
  ...
  />
    ...
    <div class="border-r border-b border-l border-gray-400 lg:border-l-0 lg:border-t lg:border-gray-400 bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
      <div class="mb-8">
        ...
        <Link to={`/initiatives_current/${algo._id}`}> <div class="text-gray-900 font-bold text-xl mb-2">{algo.name}</div></Link>
